...
const { CREATED, BAD_REQUEST } = require("../constant");
...
const RegisterController = {
  async NewRegisterPage(req, res) {
    ...
    return res.render("register/register-page", {
      layout: false,
      data,
    });
  },
  async CreateRegister(req, res) {
    ...
    if (register === "user_existed") {
      ...
      return res
        .status(200)
        .json({ code: CREATED, message: "created successfully" });
    }
    return res
      .status(200)
      .json({ code: BAD_REQUEST, message: "creating failed" });
  },
};
